...
class CustomerFilter(Filter):
    ...
    def apply(self, customers: List[Customer],
        ...
        not_valid = True
        plan = None
        if len(filter_string) != 4 or not filter_string.isnumeric():
            filtered = data
        else:
            for c in customers:
...
class DurationFilter(Filter):
    ...
    def apply(self, customers: List[Customer],
        ...
        if len(filter_string) > 4:
            return data
        elif "L" not in filter_string and "G" not in filter_string:
            return data
        elif "L" != filter_string[0] and "G" != filter_string[0]:
            return data
        else:
            if "L" in filter_string:
                dur = int(filter_string[1:])
                for call in data:
                    if call.duration < dur:
                        filtered.append(call)
            elif "G" in filter_string:
                dur = int(filter_string[1:])
                for call in data:
                    if call.duration > dur:
                        filtered.append(call)
...
class LocationFilter(Filter):
    ...
    def apply(self, customers: List[Customer],
        ...
        if len(filters) != 4:
            return data
        for coord in filters:
            if coord.count('.') > 1:
                return data
            temp = coord.replace(".", "").replace(" ", "").replace("-", "")
            if not temp.isnumeric():
                return data
        coords = ([-float(filters[0].replace("-", "")),
                   -float(filters[2].replace(" ", "").replace("-", ""))],
                  [float(filters[1].replace(" ", "")),
                   float(filters[3].replace(" ", ""))])
        valid = True
        for c in coords[0]:
            if c < -79.697878 or c > -79.196382:
                valid = False
        for c in coords[1]:
            if c < 43.576959 or c > 43.799568:
                valid = False
        if not valid:
            return data
        for call in data:
